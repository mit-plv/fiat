...
Ltac get_ithDefault f n k :=
  match type of f with
    | ilist (@Tuple) ?A -> ?C =>
      ...
      (fun b : ilist2 (@RawTuple) A => ith2 unit_Tuple b n) in
          evar (G : @RawTuple (nth_default unit_Heading A n) -> C);
        assert (H : forall p,
                      f p = G (proj p)) by
          (subst G; intro p;
...
Ltac findGoodTerm_dep SC F indexed_attrs visited_attrs makeClause k :=
  match F with
    | fun (a : ?T) b => ?[@?f a b] =>
      match type of f with
        | forall a b, {@?X a = GetAttribute _ ?fd'} + {_} =>
        ...
        | forall a b, {GetAttribute _ ?fd' = @?X a} + {_} =>
          let fd := eval simpl in (bindex fd') in
          let H := fresh in
          assert (List.In {| KindNameKind := EqualityIndex;
                             KindNameName := fd|} indexed_attrs) as H
...
Ltac find_equivalent_search_term_pair build_search_term_dep :=
  match goal with
      [ H : @DelegateToBag_AbsR ?qs_schema ?indices ?r_o ?r_n
        |- ExtensionalEq ?f _ ] =>
      match type of f with
        | ilist (@RawTuple) (?heading :: ?headings) -> bool =>
          makeEvar (ilist (@RawTuple) headings -> @RawTuple heading -> bool)
                   ltac:(fun filter_dec =>
                           let eqv_f_dec := fresh in
                           assert (forall a : ilist (@RawTuple) (heading :: headings),
                         ...
                         [let a := fresh in
                          intro a; let f' := match goal with |- ?f = _ => f end in
                                   let f'' :=
                                       ...
                                       | let dom := constr:(ilist (@RawTuple) headings) in
                                                                                                  ...
                                                                                                  let search_term_matcher := eval simpl in (BagMatchSearchTerm idx_search_update_term) in
                                                                                                      let search_term_type := eval unfold search_term_type' in search_term_type' in
                                                                                                          makeEvar (dom -> search_term_type)
                                                                                                               ltac: (fun search_term =>
                                                                                                                        let eqv := fresh in
                                                                                                                        assert (forall (a : ilist (@RawTuple) (headings)) (b : @RawTuple heading) ,
                                                                                                                      ...
                                                                                                                      | match goal with
                                                                                                                            ...
                                                                                                                            match type of f with
                                                                                                                              | ilist (@RawTuple) (?A :: ?As) -> _ =>
                                                                                                                                unify search_term' (fun a : ilist (@RawTuple) (A :: As) => search_term_matcher (search_term (ilist_tl a)) (ilist_hd a))
                                                                                                                            end
                                                                                                                        end;
...
Ltac remove_spurious_Dep_Type_BoundedIndex_nth_eq := idtac.
(*  match goal with
                ...
                repeat match goal with
                           |- refine (l' <- Join_Filtered_Comp_Lists ?l1 ?l2 ?cond1;
                                     ...
                                     _ => first (* No test case for this branch *)
                                            [ makeEvar (ilist (@RawTuple) headings -> bool)
                                                       ltac:(fun f' =>
                                                               find_equiv_tl heading1 headings f f';
                                                             let Comp_l2 := fresh in
                                                             assert
                                                               (forall a : ilist (@RawTuple) headings,
                         ...
                         | |- refine (l' <- Join_Filtered_Comp_Lists ?l1 ?l2 ?cond1;
                           ...
                           =>   first
                                  [ makeEvar (ilist (@RawTuple) headings -> bool) (* No test case for this branch either *)
                                             ltac:(fun f' =>
                                                     find_equiv_tl heading1 headings f f';
                                                   let Comp_l2 := fresh in
                                                   assert
            ...
            | |- refine (l' <- Join_Filtered_Comp_Lists _ _ _;
              ...
              repeat match goal with
                         |- refine (l' <- Join_Filtered_Comp_Lists ?l1 ?l2 ?cond';
                                   ...
                                   _ =>
                         first [makeEvar (ilist (@RawTuple) headings -> bool)
                                         ltac:(fun f' => find_equiv_tl heading headings f f';
                                               ...
                                               | ])
                               | etransitivity;
                                 [apply (@refine_Join_Filtered_Comp_Lists_filter_tail_andb heading headings f g ResultT resultComp l2 ) | ] ]
      ...
      match f with
        ...
        | fun a => (?MatchIndexSearchTerm (@?st a) (ilist_hd a)) && true =>
          ...
          match stT with _ -> ?stT =>
                         makeEvar (ilist (@RawTuple) headings -> stT)
                                  ltac:(fun st_dep =>
                                          let eqv := fresh in
                                          let a := fresh in
