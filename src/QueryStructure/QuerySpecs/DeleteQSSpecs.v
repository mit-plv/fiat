...
: Prop :=
  ...
  (* If deleting the tuples in [DeletedTuples] produces a
     relation consistent with the schema constraints, *)
  (forall tup tup',
  ...
  (* and each tuple in the other tables is consistent with the
     table produced by deleting the tuples in [DeletedTuples] from [Ridx], *)
  -> (forall Ridx',
        Ridx' <> Ridx ->
        forall tup',
  ...
  (* All the tuples in [DeletedTuples] are removed from the relation indexed
   by [Ridx]. The behavior of deletion is unspecified otherwise. *)
