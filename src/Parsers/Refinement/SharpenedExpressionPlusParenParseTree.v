...
Definition parser_eval_manual (str : Coq.Strings.String.string)
  : option nat.
Proof.
  refine match parser_informative str with
         | Some (SimpleParseNonTerminal nt pt)
...
Check let n := 3 in eq_refl (Some n) <: parser_eval_manual "(1+1)+1" = Some n.
Check let n := 5 in eq_refl (Some n) <: parser_eval_manual "((1+1)+(1+1))+1" = Some n.
Check let n := 55 in eq_refl (Some n) <: parser_eval_manual "(((((55)))))" = Some n.
