...
Require Import Fiat.Parsers.Reachable.OnlyLast.ReachableWellFounded.
...
Section cfg.
  ...
  Proof.
    ...
    { destruct H; [ left | right ]; eauto using reachable_from_item__of__minimal_reachable_from_item'.
      eapply maybe_empty_production__of__minimal_maybe_empty_production; [ | eassumption ]; reflexivity. }
  Defined.
    ...
    Section wf_parts.
      ...
      Section production.
        ...
        Proof.
          ...
          destruct p as [ ?? p_empty p' | ?? p' ].
          { destruct (fun k => minimal_reachable_from_item__of__reachable_from_item valid' _ p' k Hinit')
              as [ [p'' H''] | p'' ];
            [ solve [ auto with arith ]
            | left | right ].
            { exists (MinReachableProductionHead (minimal_maybe_empty_production__of__maybe_empty_production Hsub0 p_empty) p'').
              simpl in *.
