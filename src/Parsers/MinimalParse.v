...
Section cfg.
  ...
  Context (name_listT : Type)
          (initial_names_data : name_listT)
          (is_valid_name : name_listT -> string -> bool)
          (remove_name : name_listT -> string -> name_listT)
          (remove_name_1
           : forall ls ps ps',
  ...
  : name_listT -> String -> productions CharType -> Type :=
  ...
  : name_listT -> String -> production CharType -> Type :=
  ...
  : name_listT -> String -> item CharType -> Type :=
  ...
  Definition sub_name_listT (x y : name_listT) : Prop
    := forall p, is_valid_name x p = true -> is_valid_name y p = true.

  Global Instance sub_name_listT_Reflexive : Reflexive sub_name_listT
    := fun x y f => f.

  Global Instance sub_name_listT_Transitive : Transitive sub_name_listT.
  Proof.
    lazy; auto.
  Defined.

  Add Parametric Morphism : remove_name
  with signature sub_name_listT ==> eq ==> sub_name_listT
    as remove_name_mor.
  Proof.
    intros x y H z w H'.
    hnf in H.
  ...
  Lemma sub_name_listT_remove ls ps
  : sub_name_listT (remove_name ls ps) ls.
  Proof.
    intros p.
    apply remove_name_1.
  Qed.

  Lemma sub_name_listT_remove_2 ls ls' ps (H : sub_name_listT ls ls')
  : sub_name_listT (remove_name ls ps) ls'.
  Proof.
    etransitivity; eauto using sub_name_listT_remove.
  Qed.

  Lemma sub_name_listT_remove_3 ls ls' p
        (H0 : is_valid_name ls p = false)
        (H1 : sub_name_listT ls ls')
  : sub_name_listT ls (remove_name ls' p).
  Proof.
    intros p' H'.
    rewrite remove_name_5; intuition (subst; eauto; congruence).
  Qed.

  Fixpoint expand_minimal_parse_of {valid valid' str pats} (H : sub_name_listT valid valid') (p : minimal_parse_of valid str pats)
  : minimal_parse_of valid' str pats
    := match p in (minimal_parse_of valid str pats) return (sub_name_listT valid valid' -> _) with
         | MinParseHead valid str pat pats p'
           => fun H => MinParseHead pats (expand_minimal_parse_of_production H p')
         | MinParseTail valid str pat pats p'
           => fun H => MinParseTail pat (expand_minimal_parse_of H p')
       end H
  with expand_minimal_parse_of_production {valid valid' str pat} (H : sub_name_listT valid valid') (p : minimal_parse_of_production valid str pat)
       : minimal_parse_of_production valid' str pat
       := (let expand_minimal_parse_of_item {valid valid' str it} (H : sub_name_listT valid valid') (p : minimal_parse_of_item valid str it)
               := match p in (minimal_parse_of_item valid str it) return (sub_name_listT valid valid' -> minimal_parse_of_item valid' str it) with
                    | MinParseTerminal valid x
                      => fun _ => MinParseTerminal valid' x
                    | MinParseNonTerminal valid name str H p'
                      => fun H' => MinParseNonTerminal (H' _ H) (expand_minimal_parse_of (remove_name_mor H' eq_refl) p')
                  end H in
                   match p in (minimal_parse_of_production valid str pats) return (sub_name_listT valid valid' -> minimal_parse_of_production valid' str pats) with
                     | MinParseProductionNil valid
  ...
  Definition expand_minimal_parse_of_item {valid valid' str it} (H : sub_name_listT valid valid') (p : minimal_parse_of_item valid str it)
    := match p in (minimal_parse_of_item valid str it) return (sub_name_listT valid valid' -> minimal_parse_of_item valid' str it) with
         | MinParseTerminal valid x
           => fun _ => MinParseTerminal valid' x
         | MinParseNonTerminal valid name str H p'
  ...
  Section minimize.
    Let P : string -> Prop
      := fun n => is_valid_name initial_names_data n = true.
    ...
    Definition expand_alt_option {h h' str str' valid valid'}
               (H : h < h') (H' : sub_name_listT valid' valid) (H'' : str = str')
    ...
    Section wf_parts.
      Let of_parse_T' h
          {str : String} (valid : name_listT) {pats : productions CharType}
      ...
      Section item.
        Context {str : String} (valid : name_listT) {it : item CharType}.
      ...
      Section production.
        ...
        Section helper.
          ...
          Context {str : String} (valid : name_listT) {pat : production CharType}.
      ...
      Section parse_of.
        ...
        Section helper.
          ...
          Context {str : String} (valid : name_listT) {pats : productions CharType}.
