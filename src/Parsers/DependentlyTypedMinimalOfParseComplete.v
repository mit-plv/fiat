(** * Fully instantiated specialization of the dependently typed parser to minimal parse trees, taking in parse trees *)
Require Import Coq.Lists.List Coq.Program.Program Coq.Program.Wf Coq.Arith.Wf_nat Coq.Arith.Compare_dec Coq.Classes.RelationClasses Coq.Strings.String.
Require Import Parsers.ContextFreeGrammar Parsers.DependentlyTyped Parsers.MinimalParse.
Require Import Parsers.DependentlyTypedMinimalOfParse Parsers.DependentlyTypedMinimal.
...
Section recursive_descent_parser.
  ...
  Context (gen_state : forall (str0 : String) (valid : nonterminal_names_listT)
                              (prod : production CharType) (s : String),
                         split_stateT str0 valid prod s).

  Definition minimal_parse_nonterminal_name__of__parse_complete'
             (split_list_complete
              : forall str0 valid it its str pf,
                  @split_list_completeT CharType String G _ str0 valid it its str pf (split_string_for_production str0 valid it its str))
  ...
  : minimal_parse_of_name String G initial_nonterminal_names_data is_valid_nonterminal_name remove_nonterminal_name s initial_nonterminal_names_data s nonterminal_name.
    Proof.
      eapply minimal_parse_nonterminal_name__of__parse with (gen_state := gen_state); try eassumption.
      unfold H_prod_splitT'; simpl.
      move split_list_complete at bottom.
      pose (@H_prod_split').
    ...
    Section parts.
      Section common.
        Section types.
          ...
          Definition T_nonterminal_name_success (name : string) (str : StringWithSplitState String (split_stateT str0 valid (include_nonterminal_name _ name))) : Type
        ...
        Definition split_list_completeT
          ...
          := ({ s1s2 : String * String
                       & (fst s1s2 ++ snd s1s2 =s str)
                         * (minimal_parse_of_item _ G initial_nonterminal_names_data is_valid_nonterminal_name remove_nonterminal_name str0 valid (fst s1s2) it)
                         * (minimal_parse_of_production _ G initial_nonterminal_names_data is_valid_nonterminal_name remove_nonterminal_name str0 valid (snd s1s2) its) }%type)
             -> ({ s1s2 : _
                          & (In s1s2 split_list)
                            * (minimal_parse_of_item _ G initial_nonterminal_names_data is_valid_nonterminal_name remove_nonterminal_name str0 valid (fst s1s2) it)
                            * (minimal_parse_of_production _ G initial_nonterminal_names_data is_valid_nonterminal_name remove_nonterminal_name str0 valid (snd s1s2) its) }%type).
