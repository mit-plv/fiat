...
Proof.
  ...
  { intros env env' xenv data bin ext env_pm pred_pm com_pf.
    unfold compose, Bind2 in com_pf; computes_to_inv; destruct v;
      destruct v0.
    destruct (proj1 (decode1_pf (proj1 P_inv_pf)) _ _ _ _ _ (transform b0 ext) env_pm predicate_a' com_pf); intuition; simpl in *; injections.
    setoid_rewrite <- transform_assoc; rewrite H2.
    simpl.
    destruct (A_eq_dec a' a'); try congruence.
    subst.
    destruct (proj1 H5 (*decode2_pf _ (conj (eq_refl _) predicate_a') H1*)
                    _ _ _ _ _ ext H3 pred_pm com_pf');
      intuition; simpl in *; injections.
    eauto. }
  { intros.
    destruct (decode1 bin env') as [ [ [? ?] ? ] | ] eqn : ? ;
