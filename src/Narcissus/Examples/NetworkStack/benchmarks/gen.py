#!/usr/bin/env python3
"""
Rebuild microbenchmarks.ml.

The generated file uses Core_bench to measure execution times for all supported
...
def mkenc(inputname, packet, layer):
    ...
    if layer is Ether:
        ...
        return "Fiat4Mirage.fiat_arp_encode {} {} {}".format(SZ, inputname, out)
    elif layer is IP:
...
def mkdec(inputname, packet, layer):
    length = len(packet[layer])
    if layer is Ether:
        return "Fiat4Mirage.fiat_ethernet_decode {} {} {}".format(length, inputname, length)
    elif layer is ARP:
        return "Fiat4Mirage.fiat_arp_decode {} {}".format(length, inputname)
    elif layer is IP:
...
def mkbench(testname, inputname, kind, packet, layer):
    benchexpr = (mkenc if kind == "encode" else mkdec)(inputname, packet, layer)
