...
Section Delimiter.
  ...
  (* [open] should be a format in the future to support XML tag with
  attributes. *)
  ...
  Theorem delimiter_decode_correct
          (decode_A_with_term_string
            : string -> T -> CacheDecode -> option (A * T * CacheDecode))
          (A_predicate : string -> A -> Prop)
    : CorrectDecoder monoid (A_predicate close) (A_predicate close)
                     eq format_with_term_string
                     (decode_A_with_term_string close)
                     A_cache_inv
                     format_with_term_string ->
      CorrectDecoder monoid
                     (A_predicate close)
                     (A_predicate close)
                     eq
                     format_delimiter
                     (decode_delimiter decode_A_with_term_string)
                     A_cache_inv
                     format_delimiter.
  Proof.
    intros decode_A_OK.
  ...
  Definition decode_with_term_string_simple (close : string)
             (b : T) (cd : CacheDecode)
    : option (A * T * CacheDecode) :=
  ...
  Theorem decode_with_term_string_simple_correct
    : CorrectDecoder monoid A_predicate A_predicate
                     eq format_with_term_string
                     (decode_with_term_string_simple close)
                     A_cache_inv
                     format_with_term_string.
  Proof.
    eapply format_decode_correct_EquivDecoder_Proper; cycle 1.
    - unfold format_with_term_string.
      eapply format_decode_correct_refineEquiv; unfold flip.
  ...
  Definition decode_delimiter_simple :=
    decode_delimiter decode_with_term_string_simple.
  ...
  Proof.
    ...
    eapply decode_with_term_string_simple_correct.
  Qed.
