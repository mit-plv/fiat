  ...
  Proof.
    ...
    { intros env xenv xenv' l l' ext Eeq Ppredh Penc;
      ...
      { unfold Bind2 in *; computes_to_inv; subst.
        ...
        injections.
        destruct (proj1 X_decode_pf _ _ _ _ _ (mappend b0 ext0) Eeq I Penc) as [? [? ?]]; subst.
      ...
      { (* Case where the encoder decided to use compression. *)
        ...
        - destruct (proj1 X_decode_pf _ _ _ _ _ (mappend b0 ext0) Eeq I Penc') as [? [? ?]]; subst.
          ...
          eauto.
        - destruct v1.
          destruct (proj1 X_decode_pf _ _ _ _ _ (mappend b0 (mappend b1 ext0))
                          Eeq I Penc') as [? [? ?]]; subst.
          rewrite <- !mappend_assoc; simpl.
          let H' := fresh in
          destruct (Decode_w_Measure_le_eq _ _ _ X_decode_le H) as [? H'];
            rewrite H'; simpl; clear H'.
          destruct (proj1 A_decode_pf _ _ _ _ _ (mappend b1 ext0) H0 (proj1 (Ppredh _ (or_introl _ eq_refl))) Penc'') as [? [? ?]]; subst.
          let H' := fresh in
          destruct (Decode_w_Measure_lt_eq _ _ _ A_decode_lt H1) as [? H'];
            rewrite H'; simpl; clear H'.
      ...
      }
      {
        ...
        - destruct (proj1 X_decode_pf _ _ _ _ _ (mappend b0 (mappend b1 ext0)) Eeq I Penc') as [? [? ?]]; subst.
          rewrite <- !mappend_assoc; simpl.
          let H' := fresh in
          destruct (Decode_w_Measure_le_eq _ _ _ X_decode_le H) as [? H'];
            rewrite H'; simpl; clear H'.
      ...
      - destruct (A_decode b c)
        ...
        + match type of H2 with
          ...
          * destruct (getE env (a :: l)) eqn : ? .
            ...
            computes_to_econstructor; eauto.
            computes_to_econstructor; eauto.
            subst; rewrite !mappend_assoc; reflexivity.
