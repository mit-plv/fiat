...
Section reflective_forall.
  ...
  Section inner.
           ...
           (fun y else_case
            => If
                 (List.fold_right
                    ...
                    false
                    (List.map
                       (fun z => if beq (f z) y then beq x z else false)
                       all))
               Then
                 g y
               Else
    ...
    Lemma forall_enumerable_by_beq_aggregate'_eq
          (H_f_good : forall y, f x = f y -> P y -> P x)
      : forall_enumerable_by_beq_aggregate = forall_enumerable_by_beq_aggregate'.
    Proof.
      unfold forall_enumerable_by_beq_aggregate', forall_enumerable_by_beq_aggregate.
      destruct (P x) eqn:Hx.
      { match goal with
    ...
    Proof.
      rewrite forall_enumerable_by_beq_aggregate'_eq by assumption.
